...
export async function saveProjectUpdate(
  ...
  const parsed = projectUpdateSchema.safeParse({
    ...
    executiveSummary: optional(formData.get("executiveSummary")),
...
export async function sendReminder(formData: FormData): Promise<ActionResult> {
  ...
  if (!recipientProfileId || !reportingCycleId) {
    return { error: "Select who the reminder is for." };
  }
  ...
  const { error } = await supabase.from("reminders").insert({
    ...
    kind: "manual",
    message: message || "Your weekly update is outstanding for this cycle.",
  });
