...
**Active Task**: Move executive summary and expected value to the project — application and migration code COMPLETE; database script awaiting apply.
**Working On**: Nothing in code. If needed, apply `one-shared-view-update-value-impact.sql` first, then apply `supabase/apply/one-shared-view-project-content.sql`.
**Last Action**: Replaced project description with executive summary, moved expected value to projects, removed both fields from weekly updates, changed dashboard/detail/report/history reads, and added a transactional migration that archives every removed value before dropping the old columns. The apply/migration scripts now explicitly reload the PostgREST schema cache after commit. Typecheck, lint, focused whitespace checks, IDE diagnostics, and production build are clean.

**What live verification still needs**: Apply the database script, review its migrated-value output, then confirm project administration, update submission, dashboard, project detail, history, and report rendering against the migrated database.
