...
**Active Task**: "Preview the Solution" — read-only access without an NTID. Code COMPLETE (`a577d02`, `check:all` clean), all three SQL scripts APPLIED, read-only VERIFIED at the database layer.
**Working On**: Nothing blocking. One gap remains, and it is visual only: nobody has clicked through the four preview screens in a browser. The agent-browser daemon would not run in this container (`npx` missing → patched the wrapper to `bunx`, after which the daemon returned empty output and timed out), so verification was done at the HTTP/REST layer instead.
**Last Action**: Obtained a genuine preview session token and attacked the live database directly through the REST API — the exact case that disabled buttons cannot defend — confirming every write is refused and all four screens' reads work. Also corrected a wrong expected value in the preview apply script's own verification block.

**What live verification still needs**: a human (or a working browser) to confirm the four screens render, the preview banner shows on every screen and is absent from the print/PDF output, `/admin` is unreachable, and a real NTID sign-in still works with no banner.
