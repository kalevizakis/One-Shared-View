...
--
-- Contains NO personal data rows. Admins enter addresses through /admin.
...
-- Normalize before the format check sees the value.
...
-- Default privileges already grant CRUD to authenticated; keep an explicit
-- grant so a database without those defaults still works.
