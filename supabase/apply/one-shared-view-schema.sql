...
-- Reporting cycles: administrators configure them, leads may open/close.
create policy reporting_cycles_select on public.reporting_cycles
  for select to authenticated using (true);
create policy reporting_cycles_write on public.reporting_cycles
  for all to authenticated
...
-- so reporting readiness shows 80% exactly as in the mockup).
